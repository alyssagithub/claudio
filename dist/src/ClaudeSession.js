...
import { ChooseModel, NextEffort, RecordTurnOutcome, RememberModels, SupportsEffort } from "./Models.js";
const Turns = new Map();
...
function DelegateFor(Block, Model) {
    ...
    const Asked = (Block.input || {});
    if (!Asked.subagent_type || !Delegates[Asked.subagent_type]) {
        return null;
    }
...
export function AddToTurn(RequestId, ConversationId, Text, Images) {
    for (const Turn of Turns.values()) {
        ...
        Publish(Turn, {
            Parts: Turn.Parts.concat(Turn.PendingThinking.trim() !== "" ? [{ kind: "thinking", text: Turn.PendingThinking }] : [], Turn.PendingText.trim() !== "" ? [{ kind: "text", text: Turn.PendingText }] : [], [{ kind: "user", text: StripContext(Text) }]),
...
function RouteMessage(Session, Message) {
    ...
    if (Message.type === "stream_event") {
        ...
        if (Event.type === "content_block_start" && Event.content_block && Event.content_block.type === "tool_use") {
            ...
            Publish(Turn, {
                Parts: Turn.Parts.concat(Turn.PendingThinking.trim() !== "" ? [{ kind: "thinking", text: Turn.PendingThinking }] : [], Turn.PendingText.trim() !== "" ? [{ kind: "text", text: Turn.PendingText }] : [], [{ kind: "call", id: Block.id }]),
                CommittedThinking: JoinText(Turn.CommittedThinking, Turn.PendingThinking),
                CommittedText: JoinText(Turn.CommittedText, Turn.PendingText),
                PendingThinking: "",
                PendingText: "",
                    ...
                    hooks: {
                        ...
                        PostToolUse: [{
                                hooks: [async (HookInput, ToolUseId) => {
                                        ...
                                        const Finished = HookInput;
                                        const Capped = CapToolOutput(Finished.tool_name, Finished.tool_response);
                                        if (!Capped) {
                                            return {};
                                        }
...
export function StartTurn({ Text, ConversationId, Images, Model, Effort, AskForTools, GuardTools, Escalate, ExtraPrompt, FastMode, Mode, Bypass, Place, Folder }) {
...
export function DescribeTurn(Turn) {
    ...
    return {
        ...
        parts: Turn.Parts.concat(Turn.PendingThinking.trim() !== "" ? [{ kind: "thinking", text: Turn.PendingThinking }] : [], Turn.PendingText.trim() !== "" ? [{ kind: "text", text: Turn.PendingText }] : []).map((Part) => (Part.kind === "call" ? { kind: "call", call: Turn.Calls.findIndex((Call) => Call.Id === Part.id) + 1 } : Part)),
